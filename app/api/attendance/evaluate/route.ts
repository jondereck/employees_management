  ...
  try {
    ...
    const normalizedWorkSchedule = evaluationOptions?.workSchedule
      ? (() => {
          ...
          const validatedWorkingDays = Array.isArray(evaluationOptions.workSchedule.workingDays)
            ? Array.from(
                new Set(
                  evaluationOptions.workSchedule.workingDays.filter(
                    (day): day is number => Number.isInteger(day) && day >= 0 && day <= 6
                  )
                )
              ).sort((a, b) => a - b)
            : undefined;
          return { startTime, endTime, workingDays: validatedWorkingDays ?? [1, 2, 3, 4, 5] };
        })()
      : undefined;
