...
type GalleryImage = {
  ...
  useEffect(() => {
    ...
    // If current activeId still exists, keep it
    if (activeId && displayImages.some(i => i.id === activeId)) return;

    // Otherwise select newest (index 0 because we sorted desc)
    setActiveId(displayImages[0].id);
  }, [images, gender]); // eslint-disable-line react-hooks/exhaustive-deps
