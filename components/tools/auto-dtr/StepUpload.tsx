    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-2 text-sm">
          <span className="font-medium">Include Employees (IDs)</span>
          <input
            value={value.selectedEmployeeIds.join(", ")}
            onChange={(event) =>
              onChange({
                selectedEmployeeIds: event.target.value
                  .split(",")
        ...
        <label className="flex flex-col gap-2 text-sm">
          <span className="font-medium">Include Offices (IDs)</span>
          <input
            value={value.selectedOfficeIds.join(", ")}
            onChange={(event) =>
              onChange({
                selectedOfficeIds: event.target.value
                  .split(",")
                  .map((token) => token.trim())
                  .filter(Boolean),
              })
